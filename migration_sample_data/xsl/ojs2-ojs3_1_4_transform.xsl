...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- This stylesheet is a main component of OSU Emerging Technologies and Services' project
        to migrate journals from Open Journal Systems (OJS) version 2 to version 3.1.4. 
        ...
        Be advised: This transform does not handle every permutation of the OJS2 or OJS3 schemas, 
        ...
        <xsl:for-each select="collection('ojs2_exports/?select=*.xml')">
            ...
            <xsl:for-each select="//issue">
                ...
                <xsl:result-document href="ojs3_imports/{$issue_internal_id}_import.xml">
                    ...
                    <!-- create issue root element with valid OJS3 attributes -->
                    <issue xmlns="http://pkp.sfu.ca"
                        ...
                        <!-- add Public Identifier for the issue; pull this from mets_data -->
                        <id type="public" advice="update">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:value-of select="$issue_mets_record/public_id"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </id>
                        ...
                        <xsl:if test="description[text()] or cover/caption[text()]">
                            <description>
                                ...
                                <xsl:for-each select="cover/caption[text()]">
                                    <xsl:text disable-output-escaping="yes">&lt;![CDATA[&lt;p&gt;&lt;strong&gt;Cover:&lt;/strong&gt; </xsl:text>
                                    <xsl:value-of select="normalize-space(.)"
                                        disable-output-escaping="yes"/>
                        ...
                        <articles>
                            <xsl:for-each select="section/article">
                                ...
                                <article>
                                    ...
                                    <!-- create article IDs -->
                                    <!-- add Public Identifier for the article; pull this from mets_data
                                    ...
                                    <xsl:for-each select="title[text()]">
                                        <title>
                                            ...
                                            <xsl:choose>
                                                <xsl:when test="contains(., 'class=&quot;tocSectionTitle&quot;&gt;')">
                                                    <xsl:value-of select="normalize-space(substring-before(substring-after(.,'tocSectionTitle&quot;&gt;'),'&lt;/strong'))"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:choose>
                                                        <xsl:when test="contains(., '&lt;')">
                                        ...
                                        <keywords>
                                            <xsl:for-each
                                                ...
                                                <xsl:call-template name="tokenize_to_subelements">
                                                    <xsl:with-param name="delimited_string" select="$index_contents"/>
                                                    <xsl:with-param name="subelement_name" select="'keyword'"/>
                                                </xsl:call-template>
                                            </xsl:for-each>
                                        </keywords>
                                    </xsl:if>
                                    ...
                                    <xsl:if test="indexing/*[contains(name(), 'subject')]/text()">
                                        <subjects>
                                            <xsl:if test="indexing/*[contains(name(), 'subject')]/@locale">
                                                <xsl:attribute name="locale" select="@locale"/>
                                            </xsl:if>
                                            <xsl:for-each select="indexing/*[contains(name(), 'subject')]">
                                                <xsl:variable name="index_contents"
                                                    select="replace(replace(., '\p{P}$', ''), ',', ';')"/>
                                                <xsl:for-each select="tokenize($index_contents, ';')">
                                                    <subject>
                                                        <xsl:value-of select="normalize-space(.)"/>
                                                    </subject>
                                            </xsl:for-each>
                                        ...
                                        <xsl:choose>
                                            ...
                                            <xsl:when test="author/*[text()]">
                                                <xsl:for-each select="author">
                                                  ...
                                                  <xsl:if test="@primary_contact = 'true'">
                                    ...
                                    <!-- construct OJS3 submission file and article galley sections from OJS2 galley information-->
                                    
                                    <!-- submission file information -->
                                    <xsl:for-each select="*[contains(name(),'galley')][file/*[text()]]">
                                        <xsl:variable name="galley_filetype" select="file/embed/@mime_type"/>
                                        <xsl:variable name="galley_mets_record" select="$article_mets_record/files/file[@type = $galley_filetype]"/>
                                        <xsl:call-template name="submission_file">
                                            <xsl:with-param name="element_name" select="'submission_file'"/>
                                            <xsl:with-param name="genre" select="'Article Text'"/>
                                            <xsl:with-param name="file_name" select="file/embed/@filename"/>
                                            ...
                                            <xsl:with-param name="file_size" select="$galley_mets_record/file_size"/>
                                        ...
                                        <xsl:for-each-group select="htmlgalley/image" group-by="embed/@mime_type">
                                            <xsl:variable name="media_type" select="embed/@mime_type"/>
                                            <xsl:variable name="mets_group" select="$article_mets_record/files/file[@type = $media_type]"/>
                                            <xsl:for-each select="current-group()">
                                                <xsl:variable name="order" select="position()"/>
                                                <xsl:variable name="media_mets_record" select="$mets_group[position() = $order]"/>
                                                <xsl:call-template name="submission_file">
                                                    <xsl:with-param name="element_name" select="'artwork_file'"/>
                                                    ...
                                                    <xsl:with-param name="file_name" select="embed/@filename"/>
                                                    <xsl:with-param name="file_type" select="$media_type"/>
                                                    <xsl:with-param name="file_size" select="$media_mets_record/file_size"/>
                                                    <xsl:with-param name="public_id" select="substring-after($media_mets_record/public_id,'/')"/>
                                                    <xsl:with-param name="submission_file_ref" select="substring-before($media_mets_record/public_id,'/')"/>
                                    ...
                                    <!-- supplementary file information -->
                                     <xsl:for-each select="supplemental_file[file/embed/text()]">
                                        ...
                                        <xsl:variable name="supp_internal_id" select="concat($article_internal_id,'_s',format-number(position(),'00'))"/>
                                        <xsl:variable name="supp_file_mets" select="$article_mets_record/files/supp_file[@id = $supp_internal_id]"/>
                                        <xsl:call-template name="submission_file">
                                            <xsl:with-param name="element_name" select="'supplementary_file'"/>
                                            <xsl:with-param name="public_id" select="$supp_file_mets/supp_id"/>
                                            <xsl:with-param name="stage" select="'proof'"/>
                                            <xsl:with-param name="genre">
                                                <xsl:choose>
                                            ...
                                            <xsl:with-param name="file_name" select="file/embed/@filename"/>
                                            <xsl:with-param name="file_size" select="$supp_file_mets/supp_filesize"/> 
                                            <xsl:with-param name="file_type" select="$supp_file_mets/supp_type"/>
                                        </xsl:call-template>
                                    </xsl:for-each>
                                    
                                    <!-- article galley information -->
                                    <xsl:for-each select="galley[file/*[text()]]">
                                        <xsl:variable name="galley_mets_record" select="$article_mets_record/files/file[@type = 'application/pdf']"/>
                                    ...
                                    <xsl:for-each select="htmlgalley[file/*[text()]]">
                                        <xsl:variable name="galley_mets_record" select="$article_mets_record/files/file[@type = 'text/html']"/>
                                        <xsl:call-template name="article_galley">
                                    ...
                                    <xsl:for-each select="supplemental_file[file/embed/text()]">
                                        ...
                                        <xsl:call-template name="article_galley">
                                            ...
                                            <xsl:with-param name="public_id" select="$supp_file_mets/supp_id"/>
                                            <xsl:with-param name="galley_name" select="normalize-space(title)"/>
    ...
    <xsl:template name="submission_file">
        ...
        <xsl:element name="{$element_name}" xmlns="http://pkp.sfu.ca">
            ...
            <xsl:element name="revision">
                ...
                <embed>
                    <xsl:attribute name="encoding"
                        select="'base64'"/>
                    <xsl:attribute name="mime_type" select="$file_type"/>
                    <xsl:choose>
                        <xsl:when test="file">
            ...
            <xsl:if test="$element_name = 'supplementary_file'">
                <xsl:for-each select="*[not(name() = 'title' or name() = 'file' or name() = 'type_other')]">
    ...
    <xsl:template name="article_galley">
        ...
        <article_galley xmlns="http://pkp.sfu.ca">
            ...
            <submission_file_ref id="{$public_id}" revision="1"/>
        </article_galley>
    </xsl:template>
